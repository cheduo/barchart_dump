...
def login(driver, wait):
    ...
    password_field = wait.until(EC.presence_of_element_located(
        (By.ID, "login-page-form-password")))
    time.sleep(1)
    username_field.send_keys(USERNAME)
    password_field.send_keys(PASSWORD)
    
    submit_button = wait.until(EC.element_to_be_clickable(
        (By.CSS_SELECTOR, "button.bc-button.login-button")))
    submit_button.click()
    
    wait.until(EC.presence_of_element_located((By.XPATH, "//span[text()='My Account']")))
...
def setup_form_defaults(driver, wait):
    ...
    frequency_dropdown = wait.until(EC.presence_of_element_located(
    ...
    aggregation_input = wait.until(EC.presence_of_element_located((By.NAME, "aggregation")))
    ...
    volume_checkbox = wait.until(EC.element_to_be_clickable(
        (By.CSS_SELECTOR, "label[for='bc-total-volume']")))
...
login(driver, wait)
navigate_to_download_page(driver, name='VIJ25')
# Generate date ranges for download
data_range = pd.date_range(start='2010-02-01', end='2022-10-05')
